...
namespace OngekiFumenEditor.Core.Utils
{
    public static class LinqExtensionMethod
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ValueRange<T> MaxMinBy<T>(this IEnumerable<T> collection)
            => collection.MaxMinBy(x => x, Comparer<T>.Default.Compare);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ValueRange<T> MaxMinBy<T, X>(this IEnumerable<T> collection, Func<T, X> keySelector = default) where X : IComparable<X>
            => collection.MaxMinBy((a, b) => keySelector(a).CompareTo(keySelector(b)));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ValueRange<T> MaxMinBy<T>(this IEnumerable<T> collection, Func<T, T, int> comparer = default)
            => collection.MaxMinBy(x => x, comparer);

        public static ValueRange<X> MaxMinBy<T, X>(this IEnumerable<T> collection, Func<T, X> keySelector, Func<X, X, int> comparer)
        {
        ...
        public static IEnumerable<IEnumerable<T>> SplitByTurningGradient<T>(this IEnumerable<T> a, Func<T, float> valMapFunc)
        {
            ...
            while (true)
            {
                ...
                var sign = Math.Sign(CalcGradient(valMapFunc(prevPoint), valMapFunc(point)));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<ValueWithIndex<T>> WithIndex<T>(this IEnumerable<T> list)
            => list.Select((a, b) => new ValueWithIndex<T>(a, b));
