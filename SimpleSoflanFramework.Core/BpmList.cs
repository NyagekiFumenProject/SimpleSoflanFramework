...
namespace OngekiFumenEditor.Core.Base.Collections
{
    public class BpmList : IBinaryFindRangeEnumable<BPMChange, TGrid>
    {
        ...
        public struct BpmTimingPoint
        {
            public BpmTimingPoint(TimeSpan audioTime, BPMChange bpm)
            {
                AudioTime = audioTime;
                Bpm = bpm;
            }

            public TimeSpan AudioTime { get; }
            public BPMChange Bpm { get; }
        }

        private readonly List<BpmTimingPoint> cachedBpmUniformPosition = new();
        ...
        private void UpdateCachedAllBpmUniformPositionList()
        {
            ...
            cachedBpmUniformPosition.Add(new (TimeSpan.FromMilliseconds(0), prev));
            ...
            while (itor.MoveNext())
            {
                ...
                cachedBpmUniformPosition.Add(new (time, cur));
            }
        }

        public List<BpmTimingPoint> GetCachedAllBpmUniformPositionList()
        {
            var hash = 17;
        ...
        public IndexRange BinaryFindRangeIndex(TGrid min, TGrid max)
